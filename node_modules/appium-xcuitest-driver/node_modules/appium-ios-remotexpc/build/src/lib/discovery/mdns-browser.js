...
import { MDNS_MCAST_V4, MDNS_MCAST_V6, MDNS_PORT, QTYPE_A, QTYPE_AAAA, QTYPE_PTR, QTYPE_SRV, QTYPE_TXT, buildQuery, buildServiceTypeFqdn, parseMdnsMessage, } from './mdns-protocol.js';
 ...
 */
export async function browseMdnsService(serviceType, domain, timeoutMs) {
    ...
    const recordAddress = (hostName, ip) => {
    ...
    try {
        ...
        await collectResponses(sockets, timeoutMs, (data) => {
            ...
            for (const rr of records) {
                ...
                else if ((rr.type === QTYPE_A || rr.type === QTYPE_AAAA) &&
                    rr.address &&
                    rr.name) {
...
async function openMdnsSockets() {
    const handles = [];
    const errors = [];
    try {
...
async function bindIpv6Sockets() {
...
function srvEntryKey(entry) {
    return `${entry.target ?? ''}:${entry.port ?? 0}`;
}
function addSrvEntry(srvMap, instance, entry) {
...
function dedupeSrvEntries(entries) {
    const seen = new Set();
    const unique = [];
    for (const entry of entries) {
        const key = srvEntryKey(entry);
...
function logMdnsQuerySendError(err) {
    if (!err) {
        return;
    }
    log.warn(`Failed to send mDNS query to ${MDNS_MCAST_V4}:${MDNS_PORT}: ${err instanceof Error ? err.message : String(err)}`);
}
function sendQueryAll(handles, packet) {
...
function collectResponses(handles, timeoutMs, onMessage) {
    return new Promise((resolve) => {
        const onData = (chunk) => onMessage(chunk);
...
//# sourceMappingURL=mdns-browser.js.map
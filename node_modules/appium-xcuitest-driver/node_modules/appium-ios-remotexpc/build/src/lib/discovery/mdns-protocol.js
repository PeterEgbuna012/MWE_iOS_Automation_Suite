...
export function encodeName(name) {
    ...
    for (const label of trimmed ? trimmed.split('.') : []) {
        ...
        if (bytes.length > DNS_MAX_LABEL_OCTETS) {
            const preview = label.length > DNS_LABEL_PREVIEW_MAX_CHARS
                ? `${label.slice(0, DNS_LABEL_PREVIEW_PREFIX_CHARS)}…`
                : label;
            throw new Error(`DNS label "${preview}" is ${bytes.length} octets in UTF-8; ` +
...
export function decodeName(data, offset) {
    const labels = [];
    let jumped = false;
    let end = offset;
    for (let guard = 0; guard < DNS_DECODE_LOOP_LIMIT; guard += 1) {
        ...
        if ((length & DNS_NAME_COMPRESSION_PREFIX) ===
            ...
            if (pointer >= data.length) {
                throw makeDnsDecodeError(`compression pointer 0x${pointer.toString(16)} points past end of packet (length ${data.length})`, data, offset);
            }
            if (!jumped) {
        ...
        if (length > DNS_MAX_LABEL_OCTETS) {
            throw makeDnsDecodeError(`label length ${length} at offset ${offset} exceeds maximum ${DNS_MAX_LABEL_OCTETS} octets`, data, offset);
        }
        offset += 1;
        const labelEnd = offset + length;
        if (labelEnd > data.length) {
            throw makeDnsDecodeError(`label length ${length} at offset ${offset - 1} needs ${labelEnd} octets but packet has ${data.length}`, data, offset - 1);
        }
...
export function buildQuery(name, qtype, unicast = false) {
...
export function parseMdnsMessage(data) {
 ...
 */
export function decodeDnsSdInstanceName(name) {
    return name.replace(/\\(\d{3})/g, (_match, digits) => String.fromCharCode(parseInt(digits, 10)));
}
/** Build a trailing-dot FQDN from a DNS-SD type and domain (e.g. `local`). */
export function buildServiceTypeFqdn(serviceType, domain) {
...
//# sourceMappingURL=mdns-protocol.js.map
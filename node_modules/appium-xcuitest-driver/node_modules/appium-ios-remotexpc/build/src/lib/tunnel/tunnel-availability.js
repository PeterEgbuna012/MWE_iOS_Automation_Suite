...
import { TUNNEL_REGISTRY_API_BASE_PATH, TUNNEL_REGISTRY_HOST, TUNNEL_REGISTRY_HTTP_TIMEOUT_MS, TUNNEL_REGISTRY_PORT_PROBE_TIMEOUT_MS, } from './constants.js';
import { TunnelAvailabilityError } from './errors.js';
import { TunnelApiClient, } from './tunnel-api-client.js';
 ...
 */
export async function getTunnelForDevice(udid, options = {}) {
    const client = await createValidatedStrictRegistryClient();
    const waitMs = options.waitMs ?? 0;
    const entry = await client.getTunnelByUdid(udid, { waitMs });
    if (!entry) {
        throw new TunnelAvailabilityError(`No tunnel found for device ${udid}. Please run the tunnel creation script first`);
    }
 ...
 */
export async function createValidatedStrictRegistryClient() {
...
export function mapEntryToEndpoint(entry) {
    return {
        host: entry.address,
        port: entry.rsdPort,
        udid: entry.udid,
...
export function isTunnelEntryReady(entry) {
    return (entry.services !== undefined &&
        typeof entry.services === 'object' &&
        Object.keys(entry.services).length > 0);
}
async function readTunnelRegistryPort() {
    ...
    if (!Number.isInteger(port) || port < 1 || port > 65535) {
        throw new TunnelAvailabilityError(`Tunnel registry port "${stored}" is invalid; expected an integer between 1 and 65535`);
    }
    return port;
}
async function assertRegistryPortAcceptingConnections(port) {
    ...
    await new Promise((resolve, reject) => {
        ...
        socket.once('error', (err) => {
            ...
            finish(new TunnelAvailabilityError(`Tunnel registry port probe failed for ${registryAddress}: ${err.message}`, { cause: err }));
        });
    });
}
function createTunnelRegistryClient(port, options = {}) {
    return new TunnelApiClient(`http://${TUNNEL_REGISTRY_HOST}:${port}${TUNNEL_REGISTRY_API_BASE_PATH}`, options);
}
//# sourceMappingURL=tunnel-availability.js.map
...
import { extractCapabilityStrings, extractNSDictionary, extractNSKeyedArchiverObjects, hasNSErrorIndicators, isNSDictionaryFormat, } from '../dvt/utils.js';
...
export class DvtTestmanagedProxyService extends BaseService {
    static RSD_SERVICE_NAME = 'com.apple.dt.testmanagerd.remote';
    ...
    lastReceivedMessageIdByChannel = new Map();
    lastReceivedExpectsReplyByChannel = new Map();
     ...
     */
    async sendMessage(channel, selector = null, options = {}) {
    ...
    lastMessageExpectsReply(channel) {
     ...
     */
    async sendReply(channel, payload = null) {
        if (!this.socket) {
            throw new Error('Not connected to testmanagerd service');
        }
        const normalizedChannel = Math.abs(channel);
        const replyIdentifier = this.lastReceivedMessageIdByChannel.get(normalizedChannel) ??
            this.lastReceivedMessageId;
        if (!replyIdentifier) {
            throw new Error(`No received message ID available for channel ${channel}. Call recvPlist/recvMessage before sendReply.`);
        }
     ...
     */
    async recvPlist(channel = DvtTestmanagedProxyService.BROADCAST_CHANNEL, signal) {
     ...
     */
    async recvPlistWithTimeout(channel = DvtTestmanagedProxyService.BROADCAST_CHANNEL, timeoutMs = 2000) {
     ...
     */
    async recvMessage(channel = DvtTestmanagedProxyService.BROADCAST_CHANNEL, signal) {
        ...
        const compression = (payloadHeader.flags & DTX_CONSTANTS.COMPRESSION_MASK) >>
            DTX_CONSTANTS.COMPRESSION_SHIFT;
        if (compression) {
            log.debug(`Skipping compressed DTX message (type=${compression}, flags=0x${payloadHeader.flags.toString(16)}, size=${payloadHeader.totalLength})`);
            return [null, []];
        }
    ...
    async close() {
        ...
        if (activeCodes.length > 0) {
            ...
            if (this.socket?.destroyed || !this.socket?.writable) {
                ...
                try {
                    await this.sendMessage(DvtTestmanagedProxyService.BROADCAST_CHANNEL, '_channelCanceled:', { args, expectsReply: false });
                }
                catch (error) {
    ...
    async performHandshake() {
        ...
        log.debug(`Testmanagerd handshake complete. Found ${Object.keys(this.supportedIdentifiers).length} supported identifiers`);
    ...
    async recvPacketFragments(channel, signal) {
        while (true) {
     ...
     */
    parseAuxiliaryPrimitiveDictionary(buffer) {
        const values = [];
        let offset = 0;
    ...
    readPrimitiveDictEntry(buffer, offset) {
 ...
 */
export function readPrimitiveDictEntry(buffer, offset) {
    const type = buffer.readUInt32LE(offset);
    offset += 4;
    switch (type) {

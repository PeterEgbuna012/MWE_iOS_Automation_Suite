...
export var SyslogLogLevel;
(function (SyslogLogLevel) {
    SyslogLogLevel[SyslogLogLevel["Notice"] = 0] = "Notice";
    SyslogLogLevel[SyslogLogLevel["Info"] = 1] = "Info";
    SyslogLogLevel[SyslogLogLevel["Debug"] = 2] = "Debug";
    SyslogLogLevel[SyslogLogLevel["UserAction"] = 3] = "UserAction";
    SyslogLogLevel[SyslogLogLevel["Error"] = 16] = "Error";
    SyslogLogLevel[SyslogLogLevel["Fault"] = 17] = "Fault";
})(SyslogLogLevel || (SyslogLogLevel = {}));
const SYSLOG_LOG_LEVEL_NAMES = {
 ...
 */
export function parseSyslogEntry(data) {
    ...
    const imageName = imageNameSize > 1
 ...
 */
function formatTimestamp(seconds, microseconds) {
 ...
 */
export function formatSyslogEntry(entry) {
 ...
 *   Label        → cyan
 */
export function formatSyslogEntryColored(entry) {
    const ts = formatTimestamp(entry.timestampSeconds, entry.timestampMicroseconds);
    const processName = path.basename(entry.filename);
    const imageName = path.basename(entry.imageName);
    const levelColor = LOG_LEVEL_COLORS[entry.level] ?? WHITE;
    let line = `${GREEN}${ts}${RESET} ` +
...
//# sourceMappingURL=syslog-entry-parser.js.map
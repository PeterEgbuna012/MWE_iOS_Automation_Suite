...
import { buildClosePayload, buildFopenPayload, buildMkdirPayload, buildRemovePayload, buildRenamePayload, buildStatPayload, cleanupServiceSocket, createRawServiceSocket, fatalizeAfcSocket, nanosecondsToMilliseconds, parseCStringArray, parseKeyValueNullList, writeUInt64LE, } from './codec.js';
...
export class AfcService {
    ...
    constructor(udidOrSocket, silent, rsdServiceName) {
     ...
     */
    static fromSocket(socket, silent) {
    ...
    createWriteStream(handle, chunkSize) {
        return createAfcWriteStream(handle, (handlePayload, content) => this._sendAndWait(AfcOpcode.WRITE, handlePayload, content), chunkSize);
    }
    ...
    async pull(remoteSrc, localDst, options) {
        ...
        const pullSingleFile = async (remoteFilePath, localFilePath, nameAllocated = false) => {
    ...
    async _pullRecursiveInternal(remoteSrcDir, localDstDir, options, relativePath = '') {
        ...
        for (const entry of await this.listdir(remoteSrcDir)) {
            ...
            if (await this.isdir(entryPath)) {
                await this._pullRecursiveInternal(entryPath, path.join(localDirPath, localEntryName), options, entryRelativePath);
            }
            else {
     ...
     */
    _assertConnectionAlive() {
        if (this.connectionError) {
            throw this.connectionError;
        }
    }
    _markConnectionDead(error) {
        if (this.connectionError) {
            return;
        }
    ...
    _getDemux() {
        if (!this.demux) {
            this.demux = new AfcPacketDemux(async () => await this._connect(), (err) => this._markConnectionDead(err));
        }
        return this.demux;
    }
    async _sendAndWait(op, headerPayload = Buffer.alloc(0), content = Buffer.alloc(0)) {
    ...
    async _connect() {
        ...
        const resolved = await resolveTunnelService(this.udid, this.rsdServiceName, { waitMs: DEFAULT_TUNNEL_SERVICE_WAIT_MS });
     ...
     */
    async _doOperation(op, payload = Buffer.alloc(0)) {

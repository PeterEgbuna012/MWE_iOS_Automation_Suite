 ...
 */
export function sanitizeLocalFilename(name) {
    switch (os.platform()) {
        case 'win32':
 ...
 */
export function withUniqueSuffix(filename, suffix) {
...
export function appendUniqueSuffix(sanitized, suffix) {
    ...
    const reservedBytes = Buffer.byteLength(suffixPart, 'utf8') +
        Buffer.byteLength(extension, 'utf8');
    const maxBaseBytes = MAX_FILENAME_BYTES - reservedBytes;
    const truncatedBase = maxBaseBytes > 0
 ...
 */
function truncateUtf8Bytes(input, byteLength) {
    let curByteLength = 0;
 ...
 */
function replaceTrailingDotsAndSpaces(input, replacement) {
...
function stripWith(input, pattern, replacement = '') {
...
function sanitizeForDarwin(name) {
...
function sanitizeForPosix(name) {
    let sanitized = stripWith(name, POSIX_PATH_SEP_RE);
    sanitized = stripWith(sanitized, CONTROL_RE);
    sanitized = stripWith(sanitized, RESERVED_DOTS_RE);
    return finalizeSegment(sanitized);
}
//# sourceMappingURL=sanitize-local-filename.js.map
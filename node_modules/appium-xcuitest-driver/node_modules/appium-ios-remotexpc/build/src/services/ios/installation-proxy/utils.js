 ...
 */
export function createResult(action, reason, targetVersion, currentVersion) {
    return {
        action,
        reason,
        targetVersion,
 ...
 */
export function shouldInstall(isInstalled, targetVersion) {
    if (!isInstalled) {
 ...
 */
export function shouldUpgrade(isInstalled, currentVersion, targetVersion) {
    if (!isInstalled) {
    ...
    if (comparison > 0) {
        log.warn(`Current version ${currentVersion} is newer than target ${targetVersion}. Downgrades are not supported by iOS.`);
        return createResult('skipped', `Current version ${currentVersion} is newer than target ${targetVersion}. ${INSTALL_MESSAGES.DOWNGRADE_NOT_SUPPORTED}`, targetVersion, currentVersion);
    }
    log.debug(`App is already at version ${targetVersion}. Skipping reinstall.`);
    return createResult('skipped', `App is already at version ${targetVersion}. ${INSTALL_MESSAGES.SAME_VERSION_NOT_SUPPORTED}`, targetVersion, currentVersion);
}

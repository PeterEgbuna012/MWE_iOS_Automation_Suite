...
async function getDefaultSocket(opts = {}) {
    ...
    /** @type {(err: Error) => void} */
    let onError = lodash_1.default.noop;
    /** @type {() => void} */
    let onConnect = lodash_1.default.noop;
    ...
    try {
        return await (0, asyncbox_1.withTimeout)(connectPromise, timeoutMs, new Error(`Failed to connect to usbmuxd within the ${timeoutMs}ms timeout`));
    }
    catch (err) {
        if (!socket.destroyed) {
            // Avoid an unhandled 'error' event if ECONNRESET is emitted during destroy
            socket.once('error', lodash_1.default.noop);
            socket.destroy();
        }
        throw err;
    }

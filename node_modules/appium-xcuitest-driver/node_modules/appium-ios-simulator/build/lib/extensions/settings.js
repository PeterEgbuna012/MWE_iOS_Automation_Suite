...
async function updateSettings(domain, updates) {
    ...
    const argChunks = (0, utils_1.generateDefaultsCommandArgs)(updates);
...
async function configureLocalization(opts = {}) {
    ...
    const globalPrefs = {};
    let keyboardId = null;
    if (support_1.util.isPlainObject(keyboard)) {
    ...
    if (support_1.util.isPlainObject(language)) {
        const { name } = language;
        if (!name) {
    ...
    if (support_1.util.isPlainObject(locale)) {
    ...
    const argChunks = (0, utils_1.generateDefaultsCommandArgs)(globalPrefs, true);
    await Promise.all(argChunks.map((args) => this.simctl.spawnProcess(['defaults', 'write', GLOBAL_PREFS_PLIST, ...args])));
    if (keyboard && keyboardId) {
        const argChunks = (0, utils_1.generateDefaultsCommandArgs)({
            KeyboardsCurrentAndNext: [keyboardId],
            KeyboardLastUsed: keyboardId,
            KeyboardLastUsedForLanguage: { [keyboard.name]: keyboardId },
        }, true);
        await Promise.all(argChunks.map((args) => this.simctl.spawnProcess(['defaults', 'write', 'com.apple.Preferences', ...args])));
    }
    if (globalPrefs.AppleLanguages) {
...
async function updatePreferences(devicePrefs = {}, commonPrefs = {}) {
    ...
    return await PREFERENCES_PLIST_GUARD.acquire(this.constructor.name, async () => {
        const defaults = new utils_1.NSUserDefaults(plistPath);
        ...
        try {
            if (Object.keys(devicePrefs).length > 0) {
                ...
                if (await support_1.fs.exists(plistPath)) {
                    ...
                    if (support_1.util.isPlainObject(currentPlistContent.DevicePreferences) &&
                        support_1.util.isPlainObject(currentPlistContent.DevicePreferences[udidKey])) {
...
function compileSimulatorPreferences(opts = {}) {
    ...
    if (typeof connectHardwareKeyboard === 'boolean' ||
...
function verifyDevicePreferences(prefs = {}) {
    ...
    if (prefs.SimulatorWindowOrientation !== undefined &&
        ...
        throw this.log.errorWithException(`SimulatorWindowOrientation is expected to be one of ${acceptableSimulatorWindowOrientations}. ` +

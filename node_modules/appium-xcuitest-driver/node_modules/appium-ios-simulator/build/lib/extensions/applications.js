...
async function getUserInstalledBundleIdsByBundleName(bundleName) {
    ...
    const bundleInfos = (await Promise.all(bundleInfoPromises)).filter((info) => support_1.util.isPlainObject(info));
    const bundleIds = bundleInfos
        .filter(({ CFBundleName, CFBundleIdentifier }) => CFBundleName === bundleName && typeof CFBundleIdentifier === 'string')

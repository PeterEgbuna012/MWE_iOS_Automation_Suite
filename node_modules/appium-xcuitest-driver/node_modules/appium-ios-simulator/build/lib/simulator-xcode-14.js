...
class SimulatorXcode14 extends node_events_1.default {
    ...
    get uiClientBundleId() {
        return utils_1.SIMULATOR_UI_CLIENT_BUNDLE_ID;
    }
    /**
    ...
    async getUIClientPid() {
        const pid = await (0, utils_1.getMacAppPidByBundleId)(this.uiClientBundleId);
        if (pid) {
    ...
    async startUIClient(opts = {}) {
        ...
        this.log.info(`Starting UI client: ${support_1.util.quote(['open', ...args])}`);
        try {
            await (0, teen_process_1.exec)('open', args, { timeout: startUiOpts.startupTimeout });
        }
        catch (err) {
            throw new Error(`Got an unexpected error while opening UI client: ${err.stderr || err.stdout || err.message}`, { cause: err });
        }
    }
    /**
    ...
    async _getUiClientAppPath() {
        if (!this._uiClientAppPath) {
            this._uiClientAppPath = (0, utils_1.getUiClientAppPath)(this.uiClientBundleId, this.xcodeVersion);
        }

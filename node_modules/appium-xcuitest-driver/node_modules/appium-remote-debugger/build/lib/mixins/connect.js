...
async function connect(timeout = APP_CONNECT_TIMEOUT_MS) {
    ...
    try {
        ...
        if (timeout) {
            ...
            try {
                await (0, asyncbox_1.waitForCondition)(() => !support_1.util.isEmpty((0, property_accessors_1.getAppDict)(this)), {
                    waitMs: timeout,
                    intervalMs: APP_CONNECT_INTERVAL_MS,
                });
                this.log.debug(`Retrieved ${support_1.util.pluralize('application', Object.keys((0, property_accessors_1.getAppDict)(this)).length, true)} ` +
...
async function selectApp(currentUrl = null, maxTries = SELECT_APP_RETRIES, ignoreAboutBlankUrl = false) {
    ...
    const timer = new support_1.timing.Timer().start();
    if (support_1.util.isEmpty((0, property_accessors_1.getAppDict)(this))) {
    ...
    const fullPageArray = [];
    for (const [app, info] of Object.entries((0, property_accessors_1.getAppDict)(this))) {
...
function getPossibleDebuggerAppKeys(bundleIds) {
    ...
    const possibleBundleIds = support_1.util.uniq([
    ...
    return support_1.util.uniq(proxiedAppIds);
}
/**
...
function isAppIgnored(instance) {
    ...
    const appBundleIds = new Set(Object.values((0, property_accessors_1.getAppDict)(instance)).map((app) => app.bundleId));

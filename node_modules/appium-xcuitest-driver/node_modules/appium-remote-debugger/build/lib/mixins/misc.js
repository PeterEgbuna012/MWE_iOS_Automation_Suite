...
async function isJavascriptExecutionBlocked(timeoutMs = 1000) {
    try {
        await (0, asyncbox_1.withTimeout)(this.requireRpcClient().send('Runtime.evaluate', {
            expression: '1+1;',
            returnByValue: true,
            appIdKey: (0, property_accessors_1.getAppIdKey)(this),
            pageIdKey: (0, property_accessors_1.getPageIdKey)(this),
        }), timeoutMs, new utils_1.TimeoutError(`Operation timed out after ${timeoutMs}ms`));
        return false;
    }
    catch {
        return true;
...
async function garbageCollect(timeoutMs = GARBAGE_COLLECT_TIMEOUT_MS) {
    ...
    try {
        await (0, asyncbox_1.withTimeout)(this.requireRpcClient().send('Heap.gc', {
            appIdKey: (0, property_accessors_1.getAppIdKey)(this),
            pageIdKey: (0, property_accessors_1.getPageIdKey)(this),
        }), timeoutMs, new utils_1.TimeoutError(`Operation timed out after ${timeoutMs}ms`));

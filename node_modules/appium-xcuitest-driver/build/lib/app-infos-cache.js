...
const IPA_ROOT_PLIST_PATH_PATTERN = new RegExp(`^Payload/[^/]+\\.app/${(0, utils_1.escapeRegExp)(MANIFEST_FILE_NAME)}$`);
...
class AppInfosCache {
        ...
        try {
            await support_1.zip.readEntries(ipaPath, async ({ entry, extractEntryTo }) => {
                ...
                try {
                    ...
                    if ((0, utils_1.isPlainObject)(manifestPayload) && entry.uncompressedSize <= MAX_MANIFEST_SIZE) {
            ...
            this.log.debug(e.stack);
            throw new Error(`Cannot find ${MANIFEST_FILE_NAME} in '${ipaPath}'. Is it a valid application bundle?`, { cause: e });
        }
        if (!manifestPayload) {
    ...
    async _putApp(appPath) {
        ...
        if (stat.size <= MAX_MANIFEST_SIZE && (0, utils_1.isPlainObject)(payload)) {
    ...
    async _readPlist(plistPath, bundlePath) {
        try {
            ...
            this.log.debug(e.stack);
            throw new Error(`Cannot parse ${MANIFEST_FILE_NAME} of '${bundlePath}'. Is it a valid application bundle?`, { cause: e });
        }
    }
}

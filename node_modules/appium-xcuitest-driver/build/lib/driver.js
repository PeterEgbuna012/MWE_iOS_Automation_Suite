...
const app_install_1 = require("./commands/app-install");
...
const voiceOverCommands = __importStar(require("./commands/voiceover"));
const webCommands = __importStar(require("./commands/web"));
const startup_1 = require("./commands/wda/startup");
const stop_1 = require("./commands/wda/stop");
...
const remote_xpc_1 = require("./device/remote-xpc");
const wda_host_ops_1 = require("./device/wda-host-ops");
...
const constants_2 = require("./commands/wda/constants");
const app_infos_cache_1 = require("./app-infos-cache");
const context_1 = require("./commands/context");
const session_claim_handler_1 = require("./session-claim-handler");
...
class XCUITestDriver extends driver_1.BaseDriver {
    ...
    /*--------+
     | WDA    |
     +--------*/
    startWda = startup_1.start;
    /**
     * @deprecated This method should be made protected/private.
     */
    startWdaSession = startup_1.startWdaSession;
    stopWda = stop_1.stop;
    /*--------+
     | XCTEST |
     +--------+*/
    ...
    constructor(opts, shouldValidateCaps = true) {
        ...
        for (const fn of MEMOIZED_FUNCTIONS) {
            this[fn] = (0, utils_1.memoize)(this[fn]);
        }
        this.lifecycleData = {};
    ...
    async onIpcInit() {
        await session_claim_handler_1.sessionClaimHandler.registerActiveSession(this);
    }
    ...
    async createSession(w3cCaps1, w3cCaps2, w3cCaps3, driverData) {
        try {
            ...
            if ('mjpegServerFramerate' in this.opts &&
    ...
    async deleteSession(sessionId) {
        session_claim_handler_1.sessionClaimHandler.unregisterActiveSession(this);
        await helpers_1.removeAllSessionWebSocketHandlers.bind(this)();
        for (const recorder of [this._recentScreenRecorder, this._audioRecorder].filter((r) => Boolean(r))) {
            await recorder.interrupt(true);
            await recorder.cleanup();
        }
        await this._networkMonitorSession?.interrupt();
        this._networkMonitorSession = null;
        if (!(0, utils_1.isEmpty)(this._perfRecorders)) {
        ...
        await Promise.all(Object.values(this.logs).map(async (logObj) => {
            try {
    ...
    validateDesiredCaps(caps) {
        ...
        const verifyProcessArgument = (processArguments) => {
            ...
            if (env != null && !(0, utils_1.isPlainObject)(env)) {
        ...
        if (caps.processArguments) {
            ...
            else if ((0, utils_1.isPlainObject)(caps.processArguments)) {
                verifyProcessArgument(caps.processArguments);
            }
            else {
        ...
        if (caps.commandTimeouts) {
            caps.commandTimeouts = (0, helpers_1.normalizeCommandTimeouts)(caps.commandTimeouts);
        }
        ...
        if (caps.permissions) {
            try {
                for (const [bundleId, perms] of Object.entries(JSON.parse(caps.permissions))) {
                    ...
                    if (!(0, utils_1.isPlainObject)(perms)) {
    ...
    isXcodebuildNeeded() {
        return (0, constants_2.isXcodebuildNeeded)(this.opts);
    }
    // Core driver methods
    ...
    async getStatus() {
        const status = {
            ...
            build: await (0, helpers_1.getDriverInfo)(),
        };
        ...
        try {
            ...
            if (this.opts.mjpegServerPort === undefined) {
                ...
                throw new Error(`Cannot ensure MJPEG broadcast functionality by forwarding the local port ${mjpegServerPort} ` +
                    ...
                    `Original error: ${error}`, { cause: error });
            }
        }
    }
    getDefaultUrl() {
    ...
    async start() {
        ...
        this.opts.udid = udid;
        await session_claim_handler_1.sessionClaimHandler.registerActiveSession(this);
        await session_claim_handler_1.sessionClaimHandler.claimSessionUdid(this);
        ...
        if (realDevice) {
            device.attachRemoteXPCFacade(this.getOrCreateRemoteXPCFacade(true));
        }
        (0, wda_host_ops_1.assertWdaHostPlatformSupported)(this);
        if ((0, utils_1.isEmpty)(this.xcodeVersion) && (this.isXcodebuildNeeded() || this.isSimulator())) {
            // no `webDriverAgentUrl`, or on a simulator, so we need an Xcode version
            this.xcodeVersion = await (0, helpers_1.getAndCheckXcodeVersion)();
        }
        ...
        if (String(this.opts.browserName).toLowerCase() === 'safari') {
            ...
            this.opts.bundleId = helpers_1.SAFARI_BUNDLE_ID;
        ...
        if (this.opts.app) {
            await (0, helpers_1.checkAppPresent)(this.opts.app);
        ...
        this._wda = new appium_webdriveragent_1.WebDriverAgent({
            ...
            hostOps: (0, wda_host_ops_1.createWdaHostOps)(this),
        }, this.log);
        ...
        const memoizedLogInfo = (0, utils_1.memoize)(() => {
        ...
        if (this.isSimulator()) {
            if (this.opts.permissions) {
                ...
                for (const [bundleId, permissionsMapping] of Object.entries(JSON.parse(this.opts.permissions))) {
        ...
        if (this.isSafari()) {
            if ((0, helpers_1.shouldSetInitialSafariUrl)(this.opts)) {
    ...
    async configureApp() {
        ...
        this.opts.app = await this.helpers.configureApp(this.opts.app, {
            onPostProcess: app_install_1.onPostConfigureApp.bind(this),
            onDownload: app_install_1.onDownloadApp.bind(this),
            supportedExtensions: constants_1.SUPPORTED_EXTENSIONS,
        });
    }
    async determineDevice() {
        ...
        const setupVersionCaps = async () => {
            this._iosSdkVersion = await (0, helpers_1.getAndCheckIosSdkVersion)();
        ...
        if (this.opts.udid) {
            if (this.opts.udid.toLowerCase() === constants_1.UDID_AUTO) {
                try {
    ...
    async installAUT() {
        ...
        await app_install_1.verifyApplicationPlatform.bind(this)();
        ...
        if (install) {
            ...
            if (support_1.util.hasValue(this.opts.iosInstallPause)) {
                ...
                await (0, promises_1.setTimeout)(pauseMs);
            }
    ...
    async installOtherApps(otherApps) {
        ...
        const appPaths = await Promise.all(appsList.map((app) => this.helpers.configureApp(app, {
            onPostProcess: app_install_1.onPostConfigureApp.bind(this),
            onDownload: app_install_1.onDownloadApp.bind(this),
            supportedExtensions: constants_1.SUPPORTED_EXTENSIONS,
        })));
        const appIds = await Promise.all(appPaths.map((appPath) => this.appInfosCache.extractBundleId(appPath)));
        for (const [appId, appPath] of appIds.map((v, i) => [v, appPaths[i]])) {
            if (this.isRealDevice()) {
    ...
    _getCommandTimeout(cmdName) {
        if (this.opts.commandTimeouts) {
            ...
            return this.opts.commandTimeouts[helpers_1.DEFAULT_TIMEOUT_KEY];
        }
    }
    getOrCreateRemoteXPCFacade(isRealDevice) {
        ...
        if (!this._remoteXPCFacade || this._remoteXPCFacade.udid !== udid) {
            this._remoteXPCFacade = new remote_xpc_1.RemoteXPCFacade(udid, this.opts.platformVersion, this.log, isRealDevice);
        }

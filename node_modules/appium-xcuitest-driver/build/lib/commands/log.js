...
async function extractLogs(logType, logsContainer = {}) {
    ...
    if ((0, utils_1.isEmpty)(logsContainer)) {
...
async function mobileStartLogsBroadcast() {
    ...
    if (!(0, utils_1.isEmpty)(await this.server.getWebSocketHandlers(pathname))) {
    ...
    wss.on('connection', (ws, req) => {
        if (req) {
            const remoteIp = (0, utils_1.isEmpty)(req.headers['x-forwarded-for'])
        ...
        ws.on('close', (code, reason) => {
            ...
            if (!(0, utils_1.isEmpty)(code)) {
                closeMsg += ` Code: ${code}.`;
            }
            if (!(0, utils_1.isEmpty)(reason)) {
...
function assignBiDiLogListener(logEmitter, properties) {
    const { type, context = constants_1.NATIVE_WIN, srcEventName = 'output', entryTransformer } = properties;
    const listener = (logEntry) => {
        const finalEntry = entryTransformer ? entryTransformer(logEntry) : logEntry;
        this.eventEmitter.emit(constants_2.BIDI_EVENT_NAME, (0, models_1.makeLogEntryAddedEvent)(finalEntry, context, type));
    };
    logEmitter.on(srcEventName, listener);
    return [logEmitter, listener];
}
function nativeLogEntryToSeleniumEntry(x) {
    const msg = (0, utils_1.isEmpty)(x.prefix) ? x.message : `[${x.prefix}] ${x.message}`;
    return (0, helpers_1.toLogEntry)(msg.replace(COLOR_CODE_PATTERN, ''), x.timestamp ?? Date.now());
}

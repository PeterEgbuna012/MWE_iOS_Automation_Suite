...
async function mobileShake() {
    await (0, helpers_1.requireSimulator)(this, 'Shake device').shake();
}
/**
...
async function mobileScroll(name, direction, predicateString, toVisible, distance, elementId) {
    ...
    if (name) {
        ...
        if (!SUPPORTED_GESTURE_DIRECTIONS.includes(String(direction).toLowerCase())) {
...
function assertNoWebElements(actionSeq) {
    const isOriginWebElement = (gesture) => (0, utils_1.isPlainObject)(gesture) &&
        'origin' in gesture &&

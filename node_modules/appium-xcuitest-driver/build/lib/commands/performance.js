...
class PerfRecorder {
    ...
    constructor(reportRoot, udid, opts = {}) {
        ...
        this._pid = opts.pid;
        this._udid = udid;
        this._logger = support_1.logger.getLogger(`${(0, utils_1.truncateString)(this._profileName, { length: 10 })}@${this._udid.substring(0, 8)}`);
    ...
    async _enforceTermination() {
        ...
        try {
            await Promise.all([this._zippedReportPath, node_path_1.default.dirname(this._reportPath)]
                .filter(Boolean)
...
async function mobileStopPerfRecord(remotePath, user, pass, method, profileName = DEFAULT_PROFILE_NAME, headers, fileFieldName, formFields) {
    ...
    if ((0, utils_1.isEmpty)(recorders)) {
    ...
    const result = await (0, helpers_1.encodeBase64OrUpload)(resultPath, remotePath, {
        user,
        pass,
        method,
        headers,
...
async function requireXctrace() {
    ...
    try {
        ...
        throw new Error(`${XCTRACE} is not available for the active Xcode version. ` +
            `Please make sure XCode is up to date. Original error: ${e.stderr || e.message}`, { cause: e });
    }
    return xcrunPath;
}

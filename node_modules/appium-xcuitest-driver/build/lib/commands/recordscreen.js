...
async function startRecordingScreen(options = {}) {
    ...
    if (videoQuality) {
        ...
        if (!quality) {
            throw new Error(`videoQuality value should be one of ${JSON.stringify(Object.keys(QUALITY_MAPPING))} or a number in range 1..100. ` + `'${videoQuality}' is given instead`);
        }
...
async function stopRecordingScreen(options = {}) {
    ...
    try {
        ...
        return await (0, helpers_1.encodeBase64OrUpload)(videoPath, options.remotePath, options);
    }
    finally {

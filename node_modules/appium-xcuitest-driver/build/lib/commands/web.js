...
async function getCookies() {
    ...
    return cookies.map((cookie) => {
        if (!(0, utils_1.isEmpty)(cookie.value)) {
            try {
...
function cacheWebElement(el) {
    ...
    const cacheId = elId.includes(':') ? elId : support_1.util.uuidV4();
...
function cacheWebElements(response) {
    const toCached = (v) => Array.isArray(v) || (0, utils_1.isPlainObject)(v) ? this.cacheWebElements(v) : v;
    if (Array.isArray(response)) {
        return response.map(toCached);
    }
    else if ((0, utils_1.isPlainObject)(response)) {
...
async function waitForAtom(promise) {
    ...
    const atomWaitTimeoutMs = typeof this.opts.webviewAtomWaitTimeout === 'number' && this.opts.webviewAtomWaitTimeout > 0
    ...
    const timedAtomPromise = (0, helpers_1.withTimeout)(promise, atomWaitTimeoutMs, `The atom execution has timed out after ${atomWaitTimeoutMs}ms`);
    const handlePromiseError = async (p) => {
        try {
            ...
            throw err instanceof helpers_1.TimeoutError ? await generateAtomTimeoutError.bind(this)(timer) : err;
        }
    };
    ...
    try {
        await (0, helpers_1.withTimeout)(trackedTimedAtomPromise, ATOM_INITIAL_WAIT_MS);
    }
    catch (err) {
        ...
        if (!(err instanceof helpers_1.TimeoutError) || didTimedAtomPromiseSettle) {
...
async function mobileCalibrateWebToRealCoordinatesTranslation() {
    ...
    const performCalibrationTap = async (tapX, tapY) => {
        ...
        try {
            ...
            result = (0, utils_1.isPlainObject)(title)
                ? title
                : JSON.parse(title);
        }
        catch (e) {
...
async function mobileUpdateSafariPreferences(preferences) {
    const simulator = (0, helpers_1.requireSimulator)(this, 'Updating Safari preferences');
    if (!(0, utils_1.isPlainObject)(preferences)) {
 ...
 */
function startAlertMonitorIfNeeded() {
 ...
 */
async function runAlertMonitorSession(abortController) {
    try {
...
async function alertMonitorLoop(abortController) {
    while (this._waitingAtoms.count > 0) {
        ...
        try {
            await (0, promises_1.setTimeout)(OBSTRUCTING_ALERT_PRESENCE_CHECK_INTERVAL_MS, undefined, {
...
function isValidElementIdentifier(id) {
    ...
    if (typeof id === 'string' && (0, utils_1.isEmpty)(id)) {
        return false;
    }

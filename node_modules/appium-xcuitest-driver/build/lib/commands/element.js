    ...
    if (children.length === 1) {
        ...
        switch (type) {
            case 'XCUIElementTypeTable': {
                const firstRect = await this.getElementRect(children[0]);
                const lastRect = await this.getElementRect(children.at(-1));
...
function prepareInputValue(inp) {
    if (![Array.isArray, (x) => typeof x === 'string', Number.isFinite].some((f) => f(inp))) {

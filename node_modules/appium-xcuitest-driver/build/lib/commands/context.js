...
const constants_2 = require("./bidi/constants");
...
async function getContextsAndViews(useUrl = true) {
    ...
    const ctxs = [{ id: constants_1.NATIVE_WIN, view: {} }];
    this.contexts = [constants_1.NATIVE_WIN];
...
async function activateRecentWebview() {
    ...
    const appsWithPages = Object.values(appDict).filter(({ pageArray }) => !(0, utils_1.isEmpty)(pageArray));
    if (appsWithPages.length > 0) {
        ...
        throw new Error(`The remote debugger returned ${support_1.util.pluralize('web application', Object.keys(appDict).length, true)}, ` +
...
async function listWebFrames(useUrl = true) {
    ...
    const doListPages = async (retries) => {
        try {
            ...
            if ((0, utils_1.isEmpty)(pageArray)) {
...
async function connectToRemoteDebugger() {
    ...
    this.remote.on(appium_remote_debugger_1.RemoteDebugger.EVENT_FRAMES_DETACHED, () => {
        if (!(0, utils_1.isEmpty)(this.curWebFrames)) {
...
function isWebContext() {
    return !!this.curContext && this.curContext !== constants_1.NATIVE_WIN;
}
/**
...
async function getCurrentContext() {
    if (this.curContext && this.curContext !== constants_1.NATIVE_WIN) {
        return `${WEBVIEW_BASE}${this.curContext}`;
    }
    return constants_1.NATIVE_WIN;
}
/**
...
async function setContext(name, callback, skipReadyCheck = false) {
    function alreadyInContext(desired, current) {
        return (desired === current ||
            (desired === null && current === constants_1.NATIVE_WIN) ||
            (desired === constants_1.NATIVE_WIN && current === null));
    }
    function isNativeContext(context) {
        return context === constants_1.NATIVE_WIN || context === null;
    }
    // allow the full context list to be passed in
    const strName = String(typeof name === 'object' && name && 'id' in name ? name.id : name);
    this.log.debug(`Attempting to set context to '${strName || constants_1.NATIVE_WIN}' from '${this.curContext ? this.curContext : constants_1.NATIVE_WIN}'`);
    if (alreadyInContext(strName, this.curContext) ||
        ...
        this.log.debug(`Already in '${strName || constants_1.NATIVE_WIN}' context. Doing nothing.`);
        return;
    }
...
async function getWindowHandles() {
    ...
    return (contexts
        ...
        .filter((context) => context.id !== constants_1.NATIVE_WIN)
...
async function notifyBiDiContextChange() {
    ...
    this.eventEmitter.emit(constants_2.BIDI_EVENT_NAME, (0, models_1.makeContextUpdatedEvent)(name));
}
/**
...
function isUrlIgnored(url, safariIgnoreWebHostnames) {
    if (!safariIgnoreWebHostnames || (0, utils_1.isEmpty)(safariIgnoreWebHostnames)) {
        return false;
    }
    const ignoredHosts = safariIgnoreWebHostnames
        .split(',')
        .map((b) => b.trim())
        .filter((b) => !(0, utils_1.isEmpty)(b));

...
async function findNativeElementOrElements(strategy, selector, mult, context) {
    ...
    if (strategy === '-ios predicate string') {
        ...
        strategy = css_1.WDA_CLASS_CHAIN_STRATEGY;
    }
    else if (strategy === 'css selector') {
        ({ strategy, selector } = await (0, css_1.cssToNativeLocator)(selector));
    }
...
async function doNativeFind(strategy, selector, mult, context) {
    const ctx = context ? support_1.util.unwrapElement(context) : null;
...
async function getFirstVisibleChild(mult, context) {
    ...
    let index = 1;
    while (true) {
        const strategy = css_1.WDA_CLASS_CHAIN_STRATEGY;

...
async function mobileSetContentSize(size) {
    if (!CONTENT_SIZE.includes(String(size).toLowerCase())) {
        throw new driver_1.errors.InvalidArgumentError(`The 'size' value is expected to be one of ${CONTENT_SIZE.join(',')}`);
    }
    await (0, helpers_1.requireSimulator)(this, 'Setting content size').setContentSize(size);
}
/**
...
async function mobileGetContentSize() {
    return (await (0, helpers_1.requireSimulator)(this, 'Getting content size').getContentSize());
}
